...
def get_completion(
    ...
    if model in [
        ...
        return get_completion_gpt(
            prompt, model, temperature, api_key, max_tokens, streaming
        )
    elif model in ["qwen-plus", "qwen-turbo"]:
        # 模型列表：https://help.aliyun.com/zh/model-studio/getting-started/models
        return get_completion_dashscope(
            prompt, model, temperature, api_key, max_tokens, streaming
        )
        ...
        for chunk in response:
            if hasattr(chunk, 'output') and hasattr(chunk.output, 'text'):
                ...
                if current_text and len(current_text) > len(full_response):
                    ...
                    print(new_content, end='', flush=True)  # 只输出增量部分
        return full_response
    else:
